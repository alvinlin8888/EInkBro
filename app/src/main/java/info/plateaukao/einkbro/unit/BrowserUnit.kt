...
object BrowserUnit : KoinComponent {
    const val TAG = "BrowserUnit" //AL-20231003+
    ...
    //AL-20231003+[
    fun getDestinationFolder(activity: Activity): String {
        //return Environment.DIRECTORY_DOWNLOADS
        //return activity.getFilesDir().getAbsolutePath().toString()
        return "/mnt/sdcard/Download"
    }

    fun getNetworkConnected(activity: Context): Boolean {
    ...
    /*
    fun createWiFiStatusReceiver(activity: Activity): BroadcastReceiver {
        return object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                ...
                val ni = cm.activeNetworkInfo
                val connected = ni != null && ni.state == NetworkInfo.State.CONNECTED
                Log.i(TAG, String.format("[createWiFiStatusReceiver] WiFi Status changed, act=%s, connected=%s", act, connected))
    ...
    */
    ...
    fun createDownloadReceiver(activity: Activity): BroadcastReceiver {
        return object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                ...
                Log.d(TAG, String.format("[onReceive] id=%d, file=%s", downloadFileId, downloadManager.getUriForDownloadedFile(downloadFileId))) //AL-20231003+
                //AL-20231003*[
                //val mimeType: String = downloadManager.getMimeTypeForDownloadedFile(downloadFileId)
                var mimeType: String = ""
                try {
                    mimeType = downloadManager.getMimeTypeForDownloadedFile(downloadFileId)
                }
                catch (ex: Exception) {
                    Log.e(TAG, String.format("Exception=%s", ex.toString()))
                }
                //AL-20231003*]
    ...
    fun openDownloadFolder(activity: Activity) {
        val uri = Uri.parse(
            //Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS) //AL-20231003-
            getDestinationFolder(activity) //AL-20231003+
        )
        Log.d(TAG, String.format("[openDownloadFolder] uri=%s", uri.toString())) //AL-20231003+
        val intent =
    ...
    fun download(context: Context, url: String, contentDisposition: String, mimeType: String) {
        ...
        Log.d(TAG, String.format("[download] url=%s, filename=%s", url, filename)) //AL-20231003+
    ...
    private fun internalDownload(
        ...
        Log.d(TAG, String.format("[internalDownload] url=%s, filename=%s", url, filename)) //AL-20231003+
        val request = DownloadManager.Request(Uri.parse(url)).apply {
            ...
            getDestinationFolder(activity) + "/" + filename //AL-20231003+
        }
